function CreateINPfilefrom3DDataset(M4Nodes, M4Elements, StepBCs, MatP, INPWriteFolder)
%| Inputs:
...
disp('1.  Writing Nodes');
Nodes     = cell(length(M4Nodes)+9,1);
Nodes(1)  = cellstr('*Heading'); 
Nodes(2)  = cellstr(['** Job name: ' MatP.unique 'Model name: Model-1']); 
...
%%
disp('3.  Writing Materials Orientation and Assembly');
Assmp     =  cell(16,1);
...
%%
disp('4.  Writing instances and node lists');
...
%% Materials Prop
disp('5.  Writing Materil propreties');
Mate     =  cell(17,1);
...
elseif MatP.type == 'R'
    ...
    MatP.E = MatP.E*MatP.offset^2;       MatP.yield = MatP.yield*MatP.offset^2;
...
disp('6.  Writing Boundary Conditions');
...
fprintf('7.  Writing BC .. ');
BCf = [INPWriteFolder '/' MatP.unique '_Model_Cont' ...
        num2str(ceil(min(size(MatP.Maps.X1))*0.5-2)) '.inp'];
fileID = fopen(BCf,'w');
