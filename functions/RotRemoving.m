function [theta,Ux,Uy,rotCentre] = RotRemoving(centre,X,Y,Ux,Uy)
...
if ~strcmpi(centre,'norot')
    ...
    [theta,~] = lsqnonlin(thetaSolver2D,theta,-2*pi, 2*pi,options2);%theta);
    disp '***********'
...
%% 6. Define outputs
theta=rad2deg(theta);
UX = reshape(big_new_data_natural(:,3),length(unique(Y)),length(unique(X)));
UY = reshape(big_new_data_natural(:,4),length(unique(Y)),length(unique(X)));

%% 7. plot
figure
s1=subplot(2,2,1);  	imagesc(unique(X),unique(Y),Ux); 	set(gca,'Ydir','normal'); 
...
s1=subplot(2,2,2);  	imagesc(unique(X),unique(Y),Uy);	set(gca,'Ydir','normal'); 
s1.XDir='reverse';   s1.YDir='reverse'; axis image;axis xy; colormap jet;%axis off;
xlabel('X[\mum]');  	ylabel('Y[\mum]');   
c = colorbar;        c.Label.String = 'Ori. Uy[\mum]';    v2 = caxis;

s1=subplot(2,2,3);  	imagesc(unique(X),unique(Y),UX); 	set(gca,'Ydir','normal'); 
s1.XDir='reverse';   s1.YDir='reverse'; axis image;axis xy; colormap jet;%axis off;
xlabel('X[\mum]');  	ylabel('Y[\mum]');   
c = colorbar;        c.Label.String = 'Cor. Ux[\mum]';     caxis(v1);

s1=subplot(2,2,4);  	imagesc(unique(X),unique(Y),UY); 	set(gca,'Ydir','normal'); 
s1.XDir='reverse';   s1.YDir='reverse'; axis image;axis xy; colormap jet;%axis off;
xlabel('X[\mum]');  	ylabel('Y[\mum]');   
